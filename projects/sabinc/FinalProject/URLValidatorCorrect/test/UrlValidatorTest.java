...
*/
   public void testIsValid(Object[] testObjects, long options) {
      UrlValidator urlVal = new UrlValidator(null, null, options); //TODO:look into
      ...
      do {
		 ...
		 //loop through testPartsIndex 
		 // int[] testPartsIndex = {0, 0, 0, 0, 0};
         for (int testPartsIndexIndex = 0; testPartsIndexIndex < testPartsIndex.length; ++testPartsIndexIndex) {//loop through testPartsIndex
            int index = testPartsIndex[testPartsIndexIndex]; //set index (starts as 0,0,0,0,0)
            ResultPair[] part = (ResultPair[]) testObjects[testPartsIndexIndex];//Create part of URL
            testBuffer.append(part[index].item);//add part of URL to testBuffer
            expected &= part[index].valid;//Tell whether we expect to be valid or not
         }
         //System.out.println(testPartsIndex[0]);
         String url = testBuffer.toString();//create URL
         boolean result = urlVal.isValid(url);//get result of check
         
         if(result == true)
        	 System.out.println(url);
		 
		 //did test match what is expected
         assertEquals(url, expected, result);//did url pass?
   ...
   public void testValidator218() {
	...
	// int[] testPartsIndex = {0, 0, 0, 0, 0}; global
	//testUrlParts == testObjects
	// Object[] testUrlParts = {testUrlScheme, testUrlAuthority, testUrlPort, testPath, testUrlQuery};
   static boolean incrementTestPartsIndex(int[] testPartsIndex, Object[] testParts) {
      boolean carry = true;  //add 1 to lowest order part.
      boolean maxIndex = true;
      for (int testPartsIndexIndex = testPartsIndex.length - 1; testPartsIndexIndex >= 0; --testPartsIndexIndex) {//loop though testPartsIndex 5
         int index = testPartsIndex[testPartsIndexIndex];
         ResultPair[] part = (ResultPair[]) testParts[testPartsIndexIndex];//part at 0 is 
         if (carry) {
